  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        const res = await fetch(`http://localhost:8000/stats/${encodedId}`)
        if (res.ok) {
    ...
    >
      ...
      >
        {[
          ...
          { icon: PictureInPicture, label: 'PIP', action: () => addToast({ title: 'Picture-in-Picture', message: 'Feature coming soon.', type: 'info' }) },
