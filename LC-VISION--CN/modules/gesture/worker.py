                ...
                for h in associated_hands:
                    if h["gesture"] in ["Open_Palm", "Pointing_Up", "Victory"]:
                        if h.get("person_bbox"):
                            ...
                            has_hand_raise = True
                            break
                is_lobby_or_room = False
                idx = -1
                try:
                    camera_urls = config.CAMERA_URLS
                    if isinstance(camera_urls, str):
                        camera_urls = [x.strip() for x in camera_urls.split(",")]
                    idx = camera_urls.index(self.camera_id)
                    if idx in [0, 1]: # Allowed for Camera 1, 2 (Room, Lobby RTSP)
                        is_lobby_or_room = True
                except ValueError:
                    pass
                    
                if has_hand_raise:
                    logger.info(f"Hand Raise Detected on {self.camera_id} (index {idx}). is_lobby_or_room={is_lobby_or_room}")
